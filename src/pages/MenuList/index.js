...
const MenuList = (props) => {
  ...
  const [saleSearch, setSaleSearch] = useState({
    ...
    saleLast: (10000000).toLocaleString("vi", {
      style: "currency",
      currency: "VND",
    }),
  });
  ...
  const onChangeSale = (value) => {
    setSaleSearch({
      ...saleSearch,
      saleFirst: (value[0] * 100000).toLocaleString("vi", {
        style: "currency",
        currency: "VND",
      }),
      saleLast: (value[1] * 100000).toLocaleString("vi", {
        style: "currency",
        currency: "VND",
      }),
    });
    ...
    if (products) {
      result = products.map((value, index) => {
        ...
        return (
          <div
            key={index}
            className="col-sm-6 col-md-4 col-xl-3 product__items"
          >
            <Link
              onClick={() => onDetailProduct(value._id)}
              to={`/products/${value.slug}`}
              className="product__button--list"
            >
              <div className="product__content">
                <div className="product__content__img">
                  <img
                    className="img--before"
                    src={value.productImage[0].img}
                    alt=""
                  />
                  <img
                    className="img--after"
                    src={value.productImage[1].img}
                    alt=""
                  />
                </div>
      ...
      <div className="container">
        <div className="shop__header">
          <div className="filter">
            ...
            <div className="filter__right">
              {slug && !slug.message ? (
                <p>Hiển thị tất cả {slug.products.length} kết quả</p>
              ) : (
                <p>không có kết quả</p>
              )}
              <div
                onClick={() => onMenu(setFlag, flag)}
          ...
          <div className={flag === true ? "sidebar--active" : "sidebar"}>
            <div className="filter_content">
              <div className="col-xl-12 siba">
                <div className="sidebar__item">
                  <h2 className="sidebar__item--title">GIÁ TIỀN (Sale %)</h2>
                  <p className="sidebar__item--decs">
                    Sản phẩm trong khoảng:{" "}
                    <span>
                      {saleSearch.saleFirst} - {saleSearch.saleLast}{" "}
                    </span>
                  </p>
                  <Slider
                    range
        ...
        {slug && slug.status && !err ? (
          <div className="all__product">
            <div className="row">
              {loadWeb ? <LoadingProduct /> : showProductSlug(slug.products)}
            </div>
          </div>
        ) : (
