...
const Header = () => {
  ...
  const checkSearchProduct = useSelector(
    (state) => state.product.searchProduct
  );
      ...
      return (
        <li key={index} className="dropdown__item">
          <Link
            ...
            onClick={() => onCheck(setCheckSub, item)}
            to={`/menu-list/${item.slug}`}
          >
            {item.name}
          </Link>
        </li>
      ...
      result = categories.map((value, index) => {
        return (
          <li key={index} className="header__menu__list">
            <Link to={`/menu-list/${value.slug}`}>
              {value.name} <DownOutlined className="list--icon" />
            </Link>
            <ul className="dropdown">{showCategoryItem(value.children)}</ul>
          </li>
        );
      });
    }
            ...
            <ul className="mobile__list">
              <li className="sub__menu">
                <div className="mobile__item">
                  <Link
                    className={
                      checkSub === value ? "sub--icon color" : "sub--icon"
                    }
                    onClick={() => onCheck(setCheckSub, value)}
                    to={`/menu-list/${value.slug}`}
                  >
                    {value.name}{" "}
                  </Link>
                  <CaretDownFilled
                    onClick={() => onSubMenu(index, setSub)}
                    className={sub === index ? "sub--icon color" : "sub--icon"}
                  />
                </div>
                <ul
                  className={
                    sub === index ? "dropdown__sub action" : "dropdown__sub"
                  }
                >
        ...
        <div className="container">
          <div className="top">
            <div className="header__top__phone">
              <MenuOutlined
                onClick={() => onMenu(setMenu)}
                className="menu--outline"
              />
              <p className="header__phone--item">
                <FontAwesomeIcon icon={faPhone} /> +09 675 88888
              </p>
            </div>
            <div className="header__top__right">
              ...
              <Link to="/gio-hang" className="header__top_product">
                <FontAwesomeIcon icon={faShoppingCart} />{" "}
                {!useSelect || check === false ? "0" : cart} Sản phẩm
              </Link>
            </div>
          </div>
        </div>
        ...
        <div className="container">
          <nav className="nav">
            ...
            <div className="header__mid__right">
              <ul className="header__menu">{showCategory(category)}</ul>
                    ...
                    <div className="header__service">
                      <div className="header__cart__menu">
                        ...
                        <Link
                          ...
                          to="/gio-hang"
                        >
                          Giỏ Hàng ({!useSelect || check === false ? "0" : cart}
                          )
                        </Link>
                      </div>
